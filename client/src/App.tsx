...
function IconFolder() {
  return (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.4} aria-hidden="true">
      <path d="M2 4.5h4.2l1.3 1.3H14V12.5H2z" strokeLinejoin="miter" />
    </svg>
  );
}
...
function LinkBlock(props: {
  ...
  const { url, embedPage, onPinVideo } = props;
  ...
  const handleClick: React.MouseEventHandler<HTMLAnchorElement> | undefined = mediaKind && onPinVideo
    ? (e) => {
    ...
    <div className="link-block">
      ...
      {expanded ? (
        mediaKind ? (
          ...
          <LinkRichPreview url={url} embedPage />
        ) : (
...
function MessageNode(props: {
  ...
  const linkEmbedPage = m.type === "link" && (m.meta?.embed === 1 || m.meta?.embed === true);
  ...
  return (
    <article className="node">
      ...
      {m.type === "link" ? (
        <LinkBlock url={text} embedPage={linkEmbedPage} onPinVideo={onPinVideo} />
      ) : null}
...
export function App() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (ev) => {
        let data: { event?: string; message?: Message; id?: string } & Record<string, unknown>;
        try {
  ...
  const uploadOne = useCallback(
    async (a: Attachment): Promise<Message | null> => {
      ...
      return uploadEncryptedBytes(buf, a.file.name, a.file.type || "application/octet-stream");
    },
  ...
  const onPickFolder = async (list: FileList | null) => {
    ...
    try {
      ...
      copy.set(u8);
      const m = await uploadEncryptedBytes(copy.buffer, archiveName, "application/zip", {
        bundle: 1,
  ...
  return (
    <>
      ...
      >
        ...
        <div ref={shellRef} className={`shell shell--in-app${share.sharerId ? " shell--with-share" : ""}`}>
          ...
          <div className="box">
            ...
            und ein frischer Schlüssel ausgegeben. AES‑GCM passiert im Browser; der Hub kennt nur Chiffretext. Video-Link
            anklicken = links anheften. Ordner-Button bündelt zu ZIP. Siehe <code>/join</code>.
          </div>
            ...
            <div className="console">
              ...
              <footer className={`composer${sending ? " is-sending" : ""}`}>
                  ...
                  <div className={`input-shell${attachments.length > 0 ? " has-attach" : ""}`}>
                    ...
                    <div className="input-tools" role="toolbar" aria-label="attach">
                      ...
                      <button
                        ...
                        title="bundle folder as ZIP"
                        aria-label="bundle folder as ZIP"
                        disabled={sending}
                      >
                        <IconFolder />
                      </button>
                ...
                <div className="composer-foot">
                  <span className="hint">
                    shift + ↵ for newline · paste URL to embed · + queues files · folder = ZIP bundle
                  </span>
                </div>
                <input
                  ref={fileRef}
